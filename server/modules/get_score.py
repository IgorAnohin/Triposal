    ...
    def get_city_recommendation(self, n_cities=10):
        for index, row in self.new_data.iterrows():
            for key, value in self.answers.items():
                try:
                    self.scores[index] += abs(int(row[self.cities_collection._old_new_features_mapping[key]]) - int(value))
                    print(self.scores[index], int(row[self.cities_collection._old_new_features_mapping[key]]), int(value))
